...
public class AsyncKmlParser extends AsyncTask<String, Void, Bundle> {
    ...
    public AsyncKmlParser(Activity activity, Bundle option) {
        Random random = new Random();
        kmlId = "kml" + random.nextInt();
        init(activity, option);
    }

    public AsyncKmlParser(Activity activity, String kmlId, Bundle option) {
        this.kmlId = kmlId;
        init(activity, option);
    }

    private void init(Activity activity, Bundle option) {
        mActivity = activity;
        mOption = option;
            ...
            if ("networklink".equals(tagName)) {
                ...
                while (bundleIterator.hasNext()) {
                    ...
                    if ("link".equals(tagName)) {
                        AsyncKmlParser kmlParser = new AsyncKmlParser(this.mActivity, this.kmlId, mOption);
    ...
    private abstract class MyCallbackContext {

        public MyCallbackContext(String callbackId) {
            super(callbackId);
        }
    ...
    private void execOtherClassMethod(final JSONArray params) {
